...
GenericBuild.prototype.launch = function() {
    if (this.agent.conf.reuseworkfolder) {
        this.agent.ensureWorkFolder(this.s3WriteFiles.bind(this));
    } else {
        this.s1Cleanup();
    }
};
...
GenericBuild.prototype.s6AllFilesExtracted = function (err) {
    ...
    if (this.hooks.filesDone) {
        this.hooks.filesDone(this.s6ModifyConfigXML.bind(this));
...
GenericBuild.prototype.s7BuildCordova = function (err, args) {
    ...
    cordova_build.stdout.on('data', function (data) {
        ...
        _agent.log(_build, Msg.build_output, data);
    });
    ...
    cordova_build.stderr.on('data', function (data) {
        ...
        _agent.log(_build, Msg.error, data);
    });
    ...
    cordova_build.on('close', function (code) {
        ...
        if (code && code !== 1) {
            return _buildFailed('child process exited with code ' + code);
        }
        //this.s8BuildExecuted();
    }.bind(this));
};
