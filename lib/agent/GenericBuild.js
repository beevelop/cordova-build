...
GenericBuild.prototype.ensurePlatforms = function () {
    ...
    fs.exists(platform_dir, function (exists) {
        if (!exists) {
            agent.log(build, Msg.buildLog, 'Platform directory ({0}) doesnt exist... adding platform!'.format(platform_dir));
            _self.addPlatform.call(_self);
        } else {
            agent.log(build, Msg.buildLog, 'Platform directory ({0}) has been found! No need to add platform.'.format(platform_dir));
