...
AgentWorker.define({
    ...
    computeLocationFolder: function(build) {
        var agent = this;
        build.locationPath = agent.conf.reuseworkfolder ? agent.workFolder : path.resolve(agent.workFolder, build.Id());
    },
    ...
    buildIOS: function(build) {
        ...
        this.genericBuild(build, function(startBuild) {
            ...
            if (build.conf.iosskipsign) {
                ...
                var build_dir = path.resolve(agent.workfolder, buildId, "platforms/ios/build/device/");

                /* 1.) create Payload directory */
                var payload_dir = path.resolve(build_dir, "Payload");
                fs.mkdirs(payload_dir, function(err) {
                    if (err)
                        agent.log(build, Msg.error, "Can\'t create Payload directory at {2}:\n{3}", build_dir, err);

                    /* 2.) copy .app-file recursively into Payload directory */
                    var app_dir = path.resolve(build_dir, '*.app');
                    ...
                    fs.copy(app_dir, payload_dir, function(err) {
                        ...
                        fs.exists(itunesartwork, function(exists) {
                            ...
                            var cmd_zip = "/usr/bin/zip -r {1}.ipa Payload iTunesArtwork".format(product_name);
                            ...
                            exec(cmd_zip, {
                                ...
                                var pathOfIpa = path.resolve(build_dir, product_name + '.ipa'),
                                        xcodebuildLogPath = path.resolve(agent.workFolder, buildId, 'build.ios.xcodebuild.log'),
                                        pathOfInfo_plist = path.resolve(agent.workFolder, buildId, build.conf.iosprojectpath, 'Info.plist');
