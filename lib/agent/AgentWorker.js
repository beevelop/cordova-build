...
AgentWorker.prototype.onBuild = function (build) {
    ...
    build.locationPath = this.conf.reuseworkfolder ? this.resolve(this.workFolder) : path.resolve(this.workFolder, build.Id());
 ...
 */
AgentWorker.prototype.detectZipArchiver = function () {
    var agent = this;
 ...
 * @param {function} done    - callback function
 */
AgentWorker.prototype.modifyArchive = function (build, modifier, file, filenames, opts, done) {
    var agent = this;
    ...
    switch (zipArchiver) {
        case '7z':
        case '7z64':
            exec('{0} {1} -tzip {2} {3}'.format(agent.sevenZipPath, modifier, file, filenames), opts, function (err, stdout, stderr) {
                if (build.conf.status === 'cancelled') {
                    return;
                }
                if (err) {
                    return agent.buildFailed(build, errMsg, verb, filenames, into, agent.sevenZipPath, err, stderr);
                }
                done();
            });
            break;
        case 'keka7z':
            exec('/Applications/Keka.app/Contents/Resources/keka7z d -tzip {0} {1}'.format(file, filenames), opts, function (err, stdout, stderr) {
                if (build.conf.status === 'cancelled') {
                    return;
                }
                if (err) {
                    return agent.buildFailed(build, errMsg, verb, filenames, into, 'keka7z', err, stderr);
                }
                done();
            });
            break;
 ...
 * @param {function} done - callback function
 */
AgentWorker.prototype.extractArchive = function (build, file, target, opts, done) {
    var agent = this;
    switch (zipArchiver) {
        case '7z':
        case '7z64':
