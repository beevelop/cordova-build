...
Android.define({
    ...
    deleteAPKs: function(err, files) {
        files.length && this.agent.log(this.build, Msg.info, "Deleting existing apks:\n{2}", files.join('\n'));
    ...
    startBuild: function(err) {
        ...
        err && this.agent.log(this.build, Msg.info, "Error while deleting existing apks, {2}", err);
        this.agent.log(this.build, Msg.info, "Starting Android Build => GenericBuild");
        this.agent.genericBuild(this.build, this.filesDone.bind(this), this.buildDone.bind(this), null, this.command);
    },
    
    /* 3.) Hook into filesDone to make some file manipulations */
    filesDone: function(callback) {
        
        this.agent.callback = callback;
        var local_props = path.resolve(this.androidFolder, 'local.properties');
    ...
    writeLocalProperties: function(err, android_file) {
        if (err) {
            return this.agent.buildFailed(this.build, 'Error: Android exectuable could not be found! Please set ANDROID_HOME environment variable.\n{2}', err);
        }
        this.agent.log(this.build, Msg.info, 'Yeah: android exectuable has been found at {2}', android_file);
    ...
    /* x.) Success */
        ...
        !err && this.agent.buildSuccess(this.build, this.apkGlobPath.concat(['build.android.log', this.signLogPath, this.alignLogPath]));
    }
});