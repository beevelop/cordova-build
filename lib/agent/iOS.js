...
IOS.prototype.generateSignedIpa = function () {
    ...
    var pathOfInfo_plist = path.resolve(this.appDir, 'Info.plist');
    ...
    var execPath = '/usr/bin/xcrun -sdk iphoneos PackageApplication -v "{0}" -o "{1}" --sign "{2}" --embed "{3}" | tee "{4}" | egrep -A 5 -i "(return|sign|invalid|error|warning|succeeded|fail|running)"'.format(this.appDir, pathOfIpa, this.build.conf.ioscodesignidentity, this.build.conf.iosprovisioningpath, signLogPath);
...
IOS.prototype.generateUnsignedIpa = function () {
    ...
    fs.mkdirs(this.payload_dir, function (err) {
        ...
        fs.copy(this.appDir, this.payload_dir, function (err) {
            if (err) {
                this.agent.log(this.build, Msg.error, 'Can\'t copy {2} to {3}:\n{4}', path.basename(this.appDir), this.payload_dir, err);
            }
            ...
            fs.exists(this.itunesartwork, function (exists) {
                ...
                exec('/usr/bin/zip -r {0}.ipa Payload iTunesArtwork'.format(this.product_name), {
                    ...
                    var pathOfIpa = path.resolve(this.build_dir, this.product_name + '.ipa'),
                            ...
                            pathOfInfo_plist = path.resolve(this.appDir, 'Info.plist');
