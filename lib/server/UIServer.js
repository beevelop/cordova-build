 ...
 */
function UIServer(conf, cache, wwwFolder, server) {
    this.conf = conf;
    this.cache = cache;
...
UIServer.prototype.serveRelease = function (req, res) {
    ...
    res.setHeader('Content-type', mimeTypes[buildRequest.platform] || mimeTypes.ios);
...
UIServer.prototype.downloadRelease = function (req, res) {
    ...
    if (parsedBuild.platform === 'ios' && (req.params.file === 'qr' || (/iPhone|iPad|iPod/).test(req.headers['user-agent']))) {
...
UIServer.prototype.downloadManifest = function (req, res) {
    ...
    if (platform === 'ios') {
        ...
        var InfoPlist = build.outputFiles.findOne(function (file) {
            return (/Info\.plist/i).test(path.basename(file.file));
        });
        var ipaFile = build.ipaFile || new IPAFile(ipaPath, InfoPlist && InfoPlist.file);
...
UIServer.prototype.parseBuildRequest = function (req, res) {
    ...
    var build = this.server.builds[buildId];
    if (!build || !build.conf) {
...
UIServer.prototype.getInfoPlist = function (buildRequest) {
    var InfoPlist = buildRequest.build.outputFiles.findOne(function (file) {
        return (/Info\.plist/i).test(path.basename(file.file));
    });
    return InfoPlist && InfoPlist.file;
};
