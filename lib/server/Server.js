...
function Server(conf) {
    ...
    htmlFiles.forEach(function (file) {
        var file_path = path.resolve(this.www, file);
...
Server.define({
    ...
    listen: function () {
        ...
        if (conf.uiport) {
            this.startUI();
            console.log('Cordova build INTERFACE is accessible at {0}{1}{2}/'.format(conf.proxyprotocol || conf.uiprotocol || conf.protocol || 'http://', conf.proxy || conf.ui || conf.server || 'localhost', interfacePort === 80 ? '' : ':' + interfacePort));
        } else {
        ...
        if (uiOnly) {
            ...
            if (conf.uiport !== conf.port) {
                ...
                this.socket = io(this.uiHttpServer, {
        ...
        if (!uiOnly) {
            this.agents.socket = ServerSockets.agentsSocket.call(this, server);
            this.clients.socket = ServerSockets.clientsSocket.call(this, server);
            this.wwws.socket = ServerSockets.wwwsSocket.call(this, server);
    ...
    startBuildServer: function () {
        ...
        this.socket = io(this.buildHttpServer, {
            'destroy buffer size': Infinity
        });
        this.buildServerApp
