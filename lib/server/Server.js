...
function Server(conf) {
    ...
    var www = this.www = __dirname + '../../../www';
...
Server.define({
    ...
    listen: function () {
        ...
        if (conf.uiport) {
            console.log('Cordova build INTERFACE is accesible at {0}{1}{2}/'
        ...
        if (!conf.uiport) {
            console.log('Cordova build INTERFACE is disabled because you have specified -ui:false');
        }
    ...
    serveRelease: function (req, res) {
        ...
        if (platform == 'ios' && askForFile && askForFile != path.basename(ipaPath)) {
            ...
            switch (askForFile) {
                ...
                case '512.png':
                    res.writeHead(200, { 'Content-Type': 'image/png' });
                    filestream = fs.createReadStream(__dirname + '/ipa/512.png');
                    filestream.pipe(res);
                    return;
                default:
        ...
        filestream = fs.createReadStream(ipaPath);
        filestream.pipe(res);
    },
    ...
    parseBuildRequest: function (req, res) {
        ...
        if (build.master) {
            ...
            platformBuild = master.platforms.findOne(function (build) {
