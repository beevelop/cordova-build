...
if (conf.mode === 'git') {
    ...
    cmd.addArgument('ssl', { type: 'number' });
    cmd.addArgument('sslonly', { type: 'boolean' });
    
    if (conf.sslonly && (!conf.cert || !conf.key)) {
        throw new Error('-sslonly requires -cert:path/cert.pem and -key:path/key.pem to be set!');
    }
