...
ClientWorker.define({
        ...
        if (!this.buildCompleted) {
            ...
            serverUtils.readFiles(files, 'the servers-side sister of the cordova build client', function(err) {
                ...
                try {
                    ...
                    if (size) {
                        client.log(build, Msg.info, 'Uploading files to cordova build server...{2}', fileSize(size));
                    }
    ...
    'onBuildSuccess': function(build) {
        ...
        function saveBuildLog(err) {
            if (err) {
                client.log(build, Msg.debug, 'Error while cleaning up last {2} folders in CLIENT output folder {3}:\n{4}', client.conf.keep, client.location, err);
            }
            fs.writeFile(buildPath, CircularJSON.stringify(build, null, 4), done);
        }
        function done(err) {
            if (err) {
                client.log(build, Msg.debug, 'Error while saving {2}:\n{3}', buildPath, err);
            }
